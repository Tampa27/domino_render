...
def movement(game,player,players,tile):
    ...
    if isMyTurn(game.board,w,game.starter,n) == False or isPlayed or noCorrect(game,tile) or (passTile and (game.status == 'fi' or game.status == 'fg')) or (len(game.board) == 0 and passTile):
        return 
    if passTile == False:
        isCapicua = False
        if game.perPoints:

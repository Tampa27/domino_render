...
class Tournament(models.Model):
    ...
    def save(self, *args, **kwargs):
        if not self.tournament_no:
            ...
            numbers = Tournament.objects.all().order_by('table_no').values_list('table_no',flat=True)
            no = 1            
            for number in numbers:
                ...
                no += 1
        if self.inPairs:
            self.place_content_type = models.Q(app_label="dominoapp", model="pair")
        else:
            self.place_content_type = models.Q(app_label="dominoapp", model="player")

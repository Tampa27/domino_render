...
class PaymentView(viewsets.GenericViewSet, mixins.ListModelMixin):
    ...
    def get_queryset(self):
        ...
        if not user.is_superuser and not user.is_staff:
            ...
            queryset = Transaction.objects.all().exclude(type__in = ["gm","pro","tr"]).exclude(admin__user__is_superuser=False, paymentmethod = "transferencia").order_by("-time")
        if self.action in ["select", "confirm", "cancel"]:
    ...
    @extend_schema(
            operation_id="promotion_movies_2",
            request = inline_serializer(
                    name="Promotion Movies 2 Request",
                    fields={
                        "token": CharField(required=True),
                        "player_id": IntegerField(required=True)
                    }
    ...
    @action(detail=False, methods=["post"])
    def marketin_promotion(self, request):
        try:
            is_valid, message, status_response = PaymentRequest.validate_marketin_promotion(request)
            ...
            return PaymentService.process_marketin_promotion(request)        
        except Exception as e:
            return Response(data={
                "status": 'error',
                'message': str(e)}, status=409)
